...
import { X, Save, ShoppingBag, UserPlus, MapPin, Trash2, FileText, User } from 'lucide-react'
...
export default function ModalNovoAluguel({ onClose, onSave, aluguelParaEditar = null }) {
    ...
    if (aluguelParaEditar) {
        if (aluguelParaEditar.clientes) {
            setDadosCliente({
                ...
                // CORREÇÃO: Garante que os campos estão usando a variável correta
        ...
        const carregarItens = async () => {
            const { data } = await supabase.from('itens_aluguel').select('*, produtos(*)').eq('aluguel_id', aluguelParaEditar.id)
            if (data) {
                setCarrinho(data.map(item => ({
  ...
  const verificarDisponibilidadeEAdicionar = async (produto) => {
    ...
    if (aluguelParaEditar) query = query.neq('alugueis.id', aluguelParaEditar.id)
  ...
  const finalizarAluguel = async () => {
    ...
    const dadosAluguel = {
        ...
        anotacoes: anotacoes,
        status: aluguelParaEditar ? aluguelParaEditar.status : 'pendente' 
    }
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
                 ...
                 {!dadosCliente.id && !modoEdicaoCliente && (
                     <div className="relative">
                        ...
                        <input 
                            ...
                            value={termoBusca} 
                            onChange={e => handleBuscaCPF(e.target.value)} 
                            autoFocus 
                        />
                        {termoBusca.length === 14 && (
                            <button onClick={ativarNovoCliente} className="mt-2 w-full py-3 bg-blue-600 text-white font-bold rounded flex items-center justify-center gap-2 hover:bg-blue-700 transition-colors shadow-sm">
                                <UserPlus size={18}/> Cadastrar Novo Cliente
                            </button>
                        )}
                        {clienteEncontradoNaBusca && (
                            <div onClick={selecionarCliente} className="mt-2 p-3 bg-green-100 border border-green-300 rounded cursor-pointer hover:bg-green-200 flex justify-between items-center">
                                <div>
                                    <div className="font-bold text-green-800">{clienteEncontradoNaBusca.nome}</div>
                                    <div className="text-xs text-green-700">CPF: {clienteEncontradoNaBusca.cpf}</div>
                                </div>
                                <span className="text-xs font-bold bg-green-700 text-white px-2 py-1 rounded">Selecionar</span>
                            </div>
                        )}
                     </div>
                 )}
                 ...
                 {(dadosCliente.id || modoEdicaoCliente) && (
                     <div className="grid grid-cols-12 gap-3 animate-fade-in">
                         <div className="col-span-12 flex justify-end">
                             <button 
                                onClick={()=>{
                                    setDadosCliente({id:null, nome:'', cpf:'', telefone:'', rua:'', numero:'', bairro:'', cidade:''}); 
                                    setModoEdicaoCliente(false); 
                                    setTermoBusca('')
                                }} 
                         ...
                         <div className="col-span-12 md:col-span-8">
                             ...
                             <input 
                                disabled={camposIdentificacaoTravados} 
                                value={dadosCliente.nome} 
                                onChange={e => setDadosCliente({...dadosCliente, nome: e.target.value})} 
                                className={`w-full p-2 border rounded font-medium ${camposIdentificacaoTravados ? 'bg-gray-200 text-gray-600 cursor-not-allowed' : 'bg-white border-blue-300 focus:ring-2 focus:ring-blue-500'}`}
                         ...
                         <div className="col-span-12 md:col-span-4">
                             ...
                             <input 
                                ...
                                className={`w-full p-2 border rounded font-mono ${camposIdentificacaoTravados ? 'bg-gray-200 text-gray-600 cursor-not-allowed' : 'bg-white border-blue-300'}`}
                                readOnly
                             />
                         </div>
                         <div className="col-span-12 md:col-span-4">
                             <label className="text-xs font-bold text-gray-500">TELEFONE</label>
                             <input value={dadosCliente.telefone} onChange={e => handleTelefone(e.target.value)} className="w-full p-2 border rounded bg-white font-medium" placeholder="(00) 00000-0000"/>
                         </div>
                         
                         <div className="col-span-12 md:col-span-8">
                             <label className="text-xs font-bold text-gray-500 flex gap-1"><MapPin size={12}/> RUA</label>
                             <input value={dadosCliente.rua} onChange={e => setDadosCliente({...dadosCliente, rua: e.target.value})} className="w-full p-2 border rounded" placeholder="Rua / Avenida"/>
                         </div>
                         <div className="col-span-4 md:col-span-3">
                             <label className="text-xs font-bold text-gray-500">NÚMERO</label>
                             <input value={dadosCliente.numero} onChange={e => setDadosCliente({...dadosCliente, numero: e.target.value})} className="w-full p-2 border rounded" placeholder="Nº"/>
                         </div>
                         <div className="col-span-8 md:col-span-5">
                             <label className="text-xs font-bold text-gray-500">BAIRRO</label>
                             <input value={dadosCliente.bairro} onChange={e => setDadosCliente({...dadosCliente, bairro: e.target.value})} className="w-full p-2 border rounded" placeholder="Bairro"/>
                         </div>
                         <div className="col-span-12 md:col-span-4">
                             <label className="text-xs font-bold text-gray-500">CIDADE</label>
                             <input value={dadosCliente.cidade} onChange={e => setDadosCliente({...dadosCliente, cidade: e.target.value})} className="w-full p-2 border rounded" placeholder="Cidade"/>
                         </div>
                     </div>
                 )}
            </div>
            ...
            <div className="flex gap-4">
                ...
                <div className="flex-1"><label className="text-xs font-bold text-blue-600">Devolução</label><input type="date" className="w-full p-2 border rounded" value={datas.devolucao} onChange={e => setDatas({...prev, devolucao: e.target.value})}/></div>
            </div>
            ...
            {/* ANOTAÇÕES */}
            <div>
                <h4 className="font-bold text-gray-700 mb-1 flex items-center gap-2"><FileText size={16}/> Anotações</h4>
                <textarea className="w-full p-3 border border-gray-300 rounded text-sm" rows="2" placeholder="Ex: Ajustes..." value={anotacoes} onChange={e => setAnotacoes(e.target.value)}></textarea>
            </div>
        ...
        <div className="p-4 bg-gray-50 border-t flex justify-end gap-3 shrink-0">
          ...
          <button onClick={finalizarAluguel} disabled={loading || !dadosCliente.nome || carrinho.length === 0} className="px-6 py-2 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 flex items-center gap-2 shadow-lg">{loading ? <span className="animate-spin">⏳</span> : <Save size={20} />} Confirmar</button>
        </div>
      </div>
    </div>
  )
